            ...
            if (isValidDate(dateIni) && isValidDate(dateFim)) {
                ...
                if (startDate <= endDate) {
                    try {
                        ...
                        // Modifique a solicitação à API para obter todas as tarefas
                        const response = await axios.get(`${routerServer}/api/report/userTask/${userDataObject.id}`, {
                            params: formState,
                            headers: {
    ...
    const generatePDFReport = async (data: ReportData, dateIni: string, dateFin: string) => {
        ...
        const header = [
            ...
            },
            {
                ...
                text: `Relação de Requisitos por Projeto - Período: ${dateIni.split('-').reverse().join('/')} a ${dateFin.split('-').reverse().join('/')}`,
                fontSize: 16,
                bold: true,
                margin: [0, 0, 0, 10],
            }
        ...
        projects.forEach((project, index) => {
            ...
            const movementInfo: any[] = [];

            if (Array.isArray(project.users)) {
                project.users.forEach((user: any, userIndex: number) => {
                    const userName: any[] = [
                        { text: user.name },
                    ];

                    const userTasks: any[] = [
                        { text: `Requisitos Criados: ${user.tasksCreated || 0}`, style: 'tasksCreated' },
                        { text: `Requisitos Concluídos: ${user.tasksCompleted || 0}`, style: 'tasksCompleted' },
                        { text: `Requisitos Cancelados: ${user.tasksCancelled || 0}`, style: 'tasksCancelled' },
                    ];

                    // Adiciona espaço entre os nomes de usuário
                    if (userIndex > 0) {
                        movementInfo.push({ text: '', style: 'separator' });
                    }

                    // Adiciona o nome do usuário e as tarefas relacionadas
                    movementInfo.push(...userName);
                    movementInfo.push(...userTasks);
                });
            }
